...
Clients use HTTP/1.1 with a 10s timeout. Set `impersonate: chrome` on an engine to fetch through curl_cffi (Chrome TLS). Plain httpx gets 429 from Brave; impersonation does not. `search_async` / `search_sync` treat a 200 with zero parsed hits as a miss and fail over. Title + URL is enough; snippets are optional. DuckDuckGo HTML often cannot be reached at all — put a working engine first in `config.yaml`.
